...
注意，这时候的 `Dep.target` 是渲染 `watcher`，再执行 `watcher.evaluate()` 在 `get()`方法中调用了 `pushTarget()`,将 `Dep.target` 指向 `computedWatch`，然后求值收集依赖，调用 `popTarget()`将 `Dep.target` 复原，来看一下它的定义：
...
那么对于计算属性这样的 `computed watcher`，它实际上是有 2 种模式，lazy 和 active。如果 `lazy=true` 成立，则说明没有人去订阅这个 `computed watcher` 的变化，仅仅把 `this.dirty = true`，只有当下次再访问这个计算属性的时候才会重新求值。在我们的场景下，它会执行：
...
`run` 函数会重新计算，然后对比新旧值，如果变化了则执行回调函数，那么这里这个回调函数是 `noop`空函数，

1. **计算属性的 Watcher，cb 通常是一个空函数**，因为计算属性的值变化时，Vue 会自动处理依赖更新，而不需要额外的回调函数。
2. **渲染 Watcher 的 cb 通常是一个空函数，getter 为 updateComponent,即 vm.\_update(vm.\_render(), hydrating)**。
3. **Vue.prototype.$watch 的 cb 为传入的回调**
